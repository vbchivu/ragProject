...
def generate_embeddings(documents):
    ...
    for doc in documents:
        ...
        embeddings.append({'embedding': embedding, 'metadata': doc['metadata'], 'content': doc['content']})
    return embeddings

def create_faiss_index(embeddings):
    if not embeddings:
        raise ValueError("No embeddings were generated. Ensure valid documents are provided.")
    embedding_vectors = [item['embedding'] for item in embeddings]
    metadata = [{'metadata': item['metadata'], 'content': item['content']} for item in embeddings]
...
def load_model(model_choice):
    ...
    model = model_info["model_class"].from_pretrained(model_info["model_name"]).to(device)
...
def rag_system(user_query, index, metadata, model_choice, preference):
    ...
    return generate_response(prompt, tokenizer, model, requires_attention_mask, preference)
